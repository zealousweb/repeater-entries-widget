...
function zwrew_repeater_content_widget_init(){
	?>
  <h1><?php echo _e( 'Repeater Entries Widget', ZWREW_TEXT_DOMAIN ); ?></h1>
  <form method="post" action="options.php">
	<?php settings_fields( 'repeater-entries-widget-settings' ); ?>
	<?php do_settings_sections( 'repeater-entries-widget-settings' ); ?>
	<table class="form-table">
		<tr valign="top">
			<th scope="row"><?php echo _e( 'Maximum entries allowed', ZWREW_TEXT_DOMAIN ); ?>:</th>
		...
		<tr valign="top">
			<th scope="row"><?php echo _e( 'Add Image Field', ZWREW_TEXT_DOMAIN ); ?>:</th>
			<td><input type="checkbox" name="rew_image" value="1" <?php echo (get_option( 'rew_image' ) == 1) ? 'checked' : ''; ?>/></td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php echo _e( 'Add Link on Image Field', ZWREW_TEXT_DOMAIN ); ?>:</th>
			<td><input type="checkbox" name="rew_image_link" value="1" <?php echo (get_option( 'rew_image_link' ) == 1) ? 'checked' : ''; ?>/></td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php echo _e( 'Add Caption Field', ZWREW_TEXT_DOMAIN ); ?>:</th>
			<td><input type="checkbox" name="rew_caption" value="1" <?php echo (get_option( 'rew_caption' ) == 1) ? 'checked' : ''; ?>/></td>
		</tr>
	   <tr valign="top">
			<th scope="row"><?php echo _e( 'Add Link on Caption Field', ZWREW_TEXT_DOMAIN ); ?>:</th>
			<td><input type="checkbox" name="rew_caption_link" value="1" <?php echo (get_option( 'rew_caption_link' ) == 1) ? 'checked' : ''; ?>/></td>
	   </tr>
		<tr valign="top">
			<th scope="row"><?php echo _e( 'Description', ZWREW_TEXT_DOMAIN ); ?>:</th>
			<td>
				<select name = "rew_description">
					<option value="short" <?php echo (get_option( 'rew_description' ) == 'short') ? 'selected': '';?>><?php echo _e( 'Short description with external link', ZWREW_TEXT_DOMAIN ); ?></option>
					<option value="full" <?php echo (get_option( 'rew_description' ) == 'full') ? 'selected': '';?>><?php echo _e( 'Full description with Read More button', ZWREW_TEXT_DOMAIN ); ?></option>
				</select>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php echo _e( 'Maximum entries allowed', ZWREW_TEXT_DOMAIN ); ?>Target Window:</th>
			<td>
				<select name = "rew_link_target">
					<option value="_blank" <?php echo (get_option( 'rew_link_target' ) == '_blank') ? 'selected': '';?>><?php echo _e( 'New Tab', ZWREW_TEXT_DOMAIN ); ?></option>
					<option value="_window" <?php echo (get_option( 'rew_link_target' ) == '_window') ? 'selected': '';?>>
						<?php echo _e( 'New Window', ZWREW_TEXT_DOMAIN ); ?>
					</option>
					<option value="_parent" <?php echo (get_option( 'rew_link_target' ) == '_parent') ? 'selected': '';?>><?php echo _e( 'Parent Window', ZWREW_TEXT_DOMAIN ); ?></option>
				</select>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php echo _e( 'Maximum entries allowed', ZWREW_TEXT_DOMAIN ); ?><?php echo _e( 'Content Alignment', ZWREW_TEXT_DOMAIN ); ?>:</th>
			<td>
				<select name = "content_align">
					<option value="left" <?php echo (get_option( 'content_align' ) == 'left') ? 'selected': '';?>><?php echo _e( 'Left', ZWREW_TEXT_DOMAIN ); ?></option>
					<option value="center" <?php echo (get_option( 'content_align' ) == 'center') ? 'selected': '';?>><?php echo _e( 'Center', ZWREW_TEXT_DOMAIN ); ?></option>
					<option value="right" <?php echo (get_option( 'content_align' ) == 'right') ? 'selected': '';?>><?php echo _e( 'Right', ZWREW_TEXT_DOMAIN ); ?></option>
				</select>
			</td>
		</tr>
	</table>
